...
5. Create a GitHub release from that tag. Attach the ZIP and `.zip.sha256` file from `dist/`.
6. Check the repository visibility against the intended distribution, that both assets are present, the release tag points at the tested commit, and the CI run passed.
